...
class CalibrationKNewResource extends Resource
{
    ...
    // 🔥 ฟังก์ชันคำนวณอัตโนมัติ (เก็บไว้สำรอง - ไม่ใช้แล้ว)
    protected static function calculateResult($state, Set $set, Get $get)
    {
        $reading = (float) ($state ?? 0);
        $minSpec = (float) ($get('min_spec') ?? 0);
        $maxSpec = (float) ($get('max_spec') ?? 0);
        $trend = $get('trend');
        
        // 🔥 FIX: ใช้ path ที่ถูกต้อง (ออกจาก repeater item ไป root)
        $instrumentId = $get('../../../instrument_id');
        ...
        if ($trend === 'Smaller') {
            $error = $reading - $minSpec;
            $thresholdA = $minSpec + $tolerance;
            
            if ($reading < $minSpec || $reading > $maxSpec) {
                $grade = 'C';
            } elseif ($reading >= $thresholdA && $reading <= $maxSpec) {
                $grade = 'A';
            } else {
                $grade = 'B';
            }
        } elseif ($trend === 'Bigger') {
            $error = $reading - $maxSpec;
            $thresholdA = $maxSpec - $tolerance;
            
            if ($reading < $minSpec || $reading > $maxSpec) {
                $grade = 'C';
            } elseif ($reading <= $thresholdA && $reading >= $minSpec) {
                $grade = 'A';
            } else {
                $grade = 'B';
            }
        ...
        $set('error', number_format($error, 4));
        $set('Judgement', $judgement);
        $set('grade', $grade);
        
        // 🔥 Trigger overall calculation
        self::recalculateOverallStatus($set, $get);
    }
    
    // 🔥 คำนวณ Status และ Level รวม (Fixed paths)
    protected static function recalculateOverallStatus(Set $set, Get $get)
    {
        // 🔥 FIX: Path ที่ถูกต้องเพื่ออ่านข้อมูลทั้งหมด
        $readings = $get('../../../calibration_data.readings') ?? [];
        $grades = collect($readings)->pluck('grade')->filter();
        ...
        // 🔥 FIX: Path ที่ถูกต้องเพื่อ set ค่า root fields
        $set('../../../result_status', $status);
        $set('../../../cal_level', $level);
